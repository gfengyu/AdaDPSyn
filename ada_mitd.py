from data_utils import load_movie_director
import random
import numpy as np
import re
import argparse
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser('MIT-D Extraction')
    ...
    model_settings.add_argument('--alpha_r', type=float, default=1.81,
    ...
    model_settings.add_argument('--sigma_test', type=float, default=4.948,
    ...
    model_settings.add_argument('--sigma_avg', type=float, default= 1.097,
...
def construct_prompt_same(train_examples, test_example):
    ...
    for train_example in train_examples:
        ...
        prompt += "Director: " + train_example["label"] + "\n\n"
...
def construct_prompt_different(example, random_label, existing_answer):
    ...
    prompt = "Given a director for the film, generate a description accordingly and make sure to include the given director in the description.\n\n"
    for example in example:
        prompt += "Director: " + example["label"] + "\n"
        prompt += "Sentence: " + example["text"] + "\n\n"
    prompt += "Director: " + random_label + "\n"
...
if __name__ == "__main__":
    ...
    DEFAULT_NUM_TEST = 415

    train_sentences, train_labels, test_sentences, test_labels = load_movie_director()
    ...
    for n_shot in range(num_valid):
        ...
        for i in range(T_max):
            ...
            #print(count_points_within_radius(points, c, r))
