from data_utils import load_agnews
import random
import numpy as np
import re
import argparse
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser('AGNews Classification')
    ...
    model_settings.add_argument('--llambda', type=float, default=0.2,
                                help='Hyperparameter')
    model_settings.add_argument('--sigma_avg', type=float, default= 0.58,
...
def construct_prompt_different(example, random_label, existing_answer):
    prompt = "Given a label of news type, generate the chosen type of news accordingly.\n\n"
    for example in example:
        prompt += "News Type: " + label_dict[example["label"]][0] + "\n"
        prompt += "Text: " + example["text"] + "\n\n"
    prompt += "News Type: " + random_label + "\n"
...
if __name__ == "__main__":
    ...
    train_sentences, train_labels, test_sentences, test_labels = load_agnews()
    train_data = [{'text': text, 'label': label} for text, label in zip(train_sentences, train_labels)]
    train_world = [item for item in train_data if item['label'] == 0]
    train_sports = [item for item in train_data if item['label'] == 1]
    train_business = [item for item in train_data if item['label'] == 2]
    train_tech = [item for item in train_data if item['label'] == 3]
    # 4-shots
    syn_data = []
    ...
    for i in range(T_max):
        ...
        selected_private_data = random.sample(train_world, num_private_train)
        splits = list(chunks(selected_private_data, int(num_private_train / num_private_train_splits)))
        all_logprobs = []
        # Turn the data into prompts
        for split in splits:
            prompt = construct_prompt_different(split, "World", existing_answer)
        ...
        private_prompt = construct_prompt_different([], "World", existing_answer)
    ...
    for i in range(T_max):
        random.seed(sample_seed)
        selected_private_data = random.sample(train_sports, num_private_train)
        splits = list(chunks(selected_private_data, int(num_private_train / num_private_train_splits)))
        all_logprobs = []
        # Turn the data into prompts
        for split in splits:
            prompt = construct_prompt_different(split, "Sports", existing_answer)
            raw_reponse = complete(prompt, 1, model_name, temp=0, num_log_probs=20000, echo=False, n=None)
            top_logprobs = raw_reponse.logprobs.top_logprobs[0]
            normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in top_logprobs.items()}
            all_logprobs.append(normalized_top_logprobs)

        private_prompt = construct_prompt_different([], "Sports", existing_answer)
        private_raw_reponse = complete(private_prompt, 1, model_name, temp=0, num_log_probs=K, echo=False, n=None)
        private_top_logprobs = private_raw_reponse.logprobs.top_logprobs[0]
        private_normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in private_top_logprobs.items()}
    ...
    #print("Sports")
    #print(existing_answer)
    syn_data.append({'text': existing_answer, 'label': 1})

    # generate "Business"
    existing_answer = ""
    for i in range(T_max):
        random.seed(sample_seed)
        selected_private_data = random.sample(train_business, num_private_train)
        splits = list(chunks(selected_private_data, int(num_private_train / num_private_train_splits)))
        all_logprobs = []
        # Turn the data into prompts
        for split in splits:
            prompt = construct_prompt_different(split, "Business", existing_answer)
            raw_reponse = complete(prompt, 1, model_name, temp=0, num_log_probs=20000, echo=False, n=None)
            top_logprobs = raw_reponse.logprobs.top_logprobs[0]
            normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in top_logprobs.items()}
            all_logprobs.append(normalized_top_logprobs)

        private_prompt = construct_prompt_different([], "Business", existing_answer)
        private_raw_reponse = complete(private_prompt, 1, model_name, temp=0, num_log_probs=K, echo=False, n=None)
        private_top_logprobs = private_raw_reponse.logprobs.top_logprobs[0]
        private_normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in private_top_logprobs.items()}
    ...
    for i in range(T_max):
        #print(i)
        random.seed(sample_seed)
        selected_private_data = random.sample(train_tech, num_private_train)
        splits = list(chunks(selected_private_data, int(num_private_train / num_private_train_splits)))
        all_logprobs = []
        # Turn the data into prompts
        for split in splits:
            prompt = construct_prompt_different(split, "Technology", existing_answer)
            #print(prompt)
            raw_reponse = complete(prompt, 1, model_name, temp=0, num_log_probs=20000, echo=False, n=None)
            top_logprobs = raw_reponse.logprobs.top_logprobs[0]
            normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in top_logprobs.items()}
        ...
        private_prompt = construct_prompt_different([], "Technology", existing_answer)
        private_raw_reponse = complete(private_prompt, 1, model_name, temp=0, num_log_probs=K, echo=False, n=None)
        private_top_logprobs = private_raw_reponse.logprobs.top_logprobs[0]
        private_normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in private_top_logprobs.items()}
