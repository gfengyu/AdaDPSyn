...
def generate_dataset(field_name, data_path="."):
    ...
    with open(f'./data/slot-movies/train', 'r') as f:
        lines = f.readlines()
    ...
    with open(f'./data/slot-movies/test', 'r') as f:
        lines = f.readlines()
        lines = [line.replace(' <=> <NULL>','').strip() for line in lines]
    test_answers = []
    test_sentences = []
    ...
    if not os.path.isdir((f"./data/movie/{field_name}")):
        os.makedirs(f"./data/movie/{field_name}", exist_ok=True)
    train_data = {}
    train_data['content'] = train_sentences
    train_data['label'] = train_answers
    df = pd.DataFrame(train_data)
    df.to_csv(f"./data/movie/{field_name}/train.csv")
