from data_utils import load_trec
import random
import numpy as np
import re
import argparse
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser('TREC Classification')
    ...
    model_settings.add_argument('--sigma_radius', type=float, default=15,
    ...
    model_settings.add_argument('--llambda', type=float, default=0.25,
                                help='Hyperparameter')
    model_settings.add_argument('--sigma_avg', type=float, default= 1.09,
                                help='Noise multiplier for DP-avg')
    model_settings.add_argument('--model_name', type=str, default="meta-llama/Llama-2-7b-hf",
                                help='Language model')
    model_settings.add_argument('--T_max', type=int, default=15,
...
def construct_prompt_same(train_examples, test_example):
    ...
    for train_example in train_examples:
        ...
        prompt += "Answer Type: " + label_dict[train_example["label"]][0] + "\n\n"
    prompt += "Question: " + test_example["text"] + "\n"
    prompt += "Answer Type:"
    return prompt

def construct_prompt_different(example, random_label, existing_answer):
    prompt = "Given a label of answer type, generate a question based on the given answer type accordingly.\n\n"
    for example in example:
        prompt += "Answer Type: " + label_dict[example["label"]][0] + "\n"
        prompt += "Text: " + example["text"] + "\n\n"
    prompt += "Answer Type: " + random_label + "\n"
...
def query_vllm(prompt):
    ...
    predicted_label = completion.choices[0].message.content.strip().split("\n")[0].strip()
...
if __name__ == "__main__":
    ...
    label_dict = {0: ["Number"], 1: ["Location"], 2: ["Person"], 3: ["Description"], 4: ["Entity"], 5: ["Abbreviation"]}


    noise_rng = np.random.RandomState(noise_seed)

    train_sentences, train_labels, test_sentences, test_labels = load_trec()
    train_data = [{'text': text, 'label': label} for text, label in zip(train_sentences, train_labels)]
    train_num = [item for item in train_data if item['label'] == 0]
    train_person = [item for item in train_data if item['label'] == 2]
    train_descrip = [item for item in train_data if item['label'] == 3]
    train_entity = [item for item in train_data if item['label'] == 4]
    # 4-shots Number, Person, Description, Entity
    syn_data = []
    ...
    for i in range(T_max):
        ...
        selected_private_data = random.sample(train_num, num_private_train)
        splits = list(chunks(selected_private_data, int(num_private_train / num_private_train_splits)))
        all_logprobs = []
        # Turn the data into prompts
        for split in splits:
            prompt = construct_prompt_different(split, "Number", existing_answer)
        ...
        private_prompt = construct_prompt_different([], "Number", existing_answer)
    ...
    syn_data.append({'text': existing_answer, 'label': 0})

    # generate "Person"
    existing_answer = ""
    for i in range(T_max):
        random.seed(sample_seed)
        selected_private_data = random.sample(train_person, num_private_train)
        splits = list(chunks(selected_private_data, int(num_private_train / num_private_train_splits)))
        all_logprobs = []
        # Turn the data into prompts
        for split in splits:
            prompt = construct_prompt_different(split, "Person", existing_answer)
            raw_reponse = complete(prompt, 1, model_name, temp=0, num_log_probs=20000, echo=False, n=None)
            top_logprobs = raw_reponse.logprobs.top_logprobs[0]
            normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in top_logprobs.items()}
            all_logprobs.append(normalized_top_logprobs)

        private_prompt = construct_prompt_different([], "Person", existing_answer)
        private_raw_reponse = complete(private_prompt, 1, model_name, temp=0, num_log_probs=K, echo=False, n=None)
        private_top_logprobs = private_raw_reponse.logprobs.top_logprobs[0]
        private_normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in private_top_logprobs.items()}
    ...
    #print("Person")
    #print(existing_answer)
    syn_data.append({'text': existing_answer, 'label': 2})

    # generate "Description"
    existing_answer = ""
    for i in range(T_max):
        random.seed(sample_seed)
        selected_private_data = random.sample(train_descrip, num_private_train)
        splits = list(chunks(selected_private_data, int(num_private_train / num_private_train_splits)))
        all_logprobs = []
        # Turn the data into prompts
        for split in splits:
            prompt = construct_prompt_different(split, "Description", existing_answer)
            raw_reponse = complete(prompt, 1, model_name, temp=0, num_log_probs=20000, echo=False, n=None)
            top_logprobs = raw_reponse.logprobs.top_logprobs[0]
            normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in top_logprobs.items()}
            all_logprobs.append(normalized_top_logprobs)

        private_prompt = construct_prompt_different([], "Description", existing_answer)
        private_raw_reponse = complete(private_prompt, 1, model_name, temp=0, num_log_probs=K, echo=False, n=None)
        private_top_logprobs = private_raw_reponse.logprobs.top_logprobs[0]
        private_normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in private_top_logprobs.items()}
    ...
    for i in range(T_max):
        random.seed(sample_seed)
        selected_private_data = random.sample(train_entity, num_private_train)
        splits = list(chunks(selected_private_data, int(num_private_train / num_private_train_splits)))
        all_logprobs = []
        # Turn the data into prompts
        for split in splits:
            prompt = construct_prompt_different(split, "Entity", existing_answer)
            raw_reponse = complete(prompt, 1, model_name, temp=0, num_log_probs=20000, echo=False, n=None)
            top_logprobs = raw_reponse.logprobs.top_logprobs[0]
            normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in top_logprobs.items()}
            all_logprobs.append(normalized_top_logprobs)

        private_prompt = construct_prompt_different([], "Entity", existing_answer)
        private_raw_reponse = complete(private_prompt, 1, model_name, temp=0, num_log_probs=K, echo=False, n=None)
        private_top_logprobs = private_raw_reponse.logprobs.top_logprobs[0]
        private_normalized_top_logprobs = {key.replace('0x0A>', ''): value for key, value in private_top_logprobs.items()}
